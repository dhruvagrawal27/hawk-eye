...
def list_models(
    ...
    return [
        ModelInfo(
            model_id=a.model_id, layer=a.layer, version=a.version, stage=a.stage,
            signed=a.signed_valid, training_data_hash=a.training_data_hash,
            feature_set_version=a.feature_set_version, approving_reviewer=a.approving_reviewer,
            metrics=a.metrics,
        )
...
def promote_model(
    ...
    audit = AUDIT.write(
        actor=principal.user_id, actor_role=principal.role, action="model.promote",
        target=model_id,
        detail={"version": version, "stage": body.to_stage, "signoff_by": body.signoff_by,
                "canary_percent": body.canary_percent},
    )
    return PromoteResult(
        model_id=model_id, version=version, stage=body.to_stage,
        canary_percent=res["canary_percent"], signature_verified=res["signature_verified"],
