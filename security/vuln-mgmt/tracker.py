...
SLA_DAYS = {"critical": 7, "high": 30, "medium": 90, "low": 180, "negligible": 365, "unknown": 90}
...
SAMPLE = {
    "matches": [
        ...
        {"vulnerability": {"id": "CVE-2024-0002", "severity": "Medium", "fix": {"versions": ["2.1.1"]}},
...
def _sla_due(severity: str, discovered: dt.date) -> str:
    return (discovered + dt.timedelta(days=SLA_DAYS.get(severity.lower(), 90))).isoformat()
...
def normalize_grype(doc: dict) -> list[dict]:
    ...
    for mt in doc.get("matches", []):
        ...
        out.append({
            "id": v.get("id"), "package": art.get("name"), "version": art.get("version"),
            "severity": sev, "fixed_version": fix[0] if fix else None,
            "source": "grype",
        })
    return out
...
def normalize_trivy(doc: dict) -> list[dict]:
    ...
    for res in doc.get("Results", []):
        for v in res.get("Vulnerabilities", []) or []:
            out.append({
                "id": v.get("VulnerabilityID"), "package": v.get("PkgName"),
                "version": v.get("InstalledVersion"),
                "severity": (v.get("Severity") or "unknown").lower(),
                "fixed_version": v.get("FixedVersion"), "source": "trivy",
            })
    return out
...
def ingest(paths: list[str]) -> list[dict]:
    ...
    STORE.write_text(json.dumps({"updated": discovered.isoformat(), "findings": findings}, indent=2))
    return findings
...
def report() -> dict:
    ...
    return {"updated": data["updated"], "summary": summary,
            "critical_open": summary.get("critical", 0), "vapt_state": vapt,
            "findings": findings}
...
def main() -> int:
    ...
    if rep["findings"]:
        ...
        for f in rep["findings"][:20]:
            print(f"  [{f['severity']:8s}] {f['id']:18s} {f['package']}@{f['version']} "
