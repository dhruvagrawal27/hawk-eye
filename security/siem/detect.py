...
def detect_training_anomaly(events: list[dict]) -> Detection:
    ...
    return Detection(
        "TRAINING_DATA_ANOMALY", "high", worst_z >= TRAIN_ANOMALY_Z, detail
    )
...
def detect_config_change(events: list[dict]) -> Detection:
    ...
    changes = [
        e
        for e in events
        if e.get("action") in ("threshold_change", "rule_change", "config_change")
    ]
    unapproved = [e for e in changes if not e.get("four_eyes_approved")]
    hit = len(unapproved) > 0
    detail = (
        f"{len(unapproved)} unapproved config/threshold change(s): "
        f"{[e.get('target') for e in unapproved]}"
        if hit
        else f"{len(changes)} change(s), all four-eyes approved"
    )
