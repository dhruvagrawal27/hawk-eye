...
class L6Fusion:
    ...
    def __init__(self, meta_kind: str = "logistic", calibration: str = "isotonic",
                 contributing_threshold: float = 0.5) -> None:
    ...
    def fuse_one(
        ...
        *,
        ...
        contributing = [
            _LAYER_LABEL[c] for c in LAYER_COLUMNS
        ...
        return build_alert(
            alert_id=alert_id, entity_id=entity_id, calibrated_prob=cal_p, confidence=confidence,
            contributing_layers=contributing or ["L6_fusion"], reason_codes=reasons,
            exposure_inr=exposure_inr, created_ts=created_ts,
        )

    def fuse_batch(self, layer_scores: pd.DataFrame, entity_ids: Sequence[str], *,
                   alert_prefix: str = "alr") -> list[Alert]:
        ...
        for i, (eid, p) in enumerate(zip(entity_ids, cal)):
            ...
            contributing = [_LAYER_LABEL[c] for c in LAYER_COLUMNS
                            if float(row.get(c, 0.0)) >= self.contributing_threshold]
            alerts.append(build_alert(
                alert_id=f"{alert_prefix}_{i:06d}", entity_id=str(eid), calibrated_prob=float(p),
                confidence=conf, contributing_layers=contributing or ["L6_fusion"],
