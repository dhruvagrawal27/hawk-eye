...
def assemble_layer_matrix(scores: dict[str, Sequence[float]], index=None) -> pd.DataFrame:
...
class StackedMetaLearner(BaseScorer):
    ...
    def _build(self):
        if self.kind == "lightgbm":
            ...
            if lgb is not None:
                return lgb.LGBMClassifier(n_estimators=120, num_leaves=7, max_depth=3,
    ...
    def layer_contributions(self, X) -> list[dict[str, float]]:
        ...
        Xdf = (X if isinstance(X, pd.DataFrame) else assemble_layer_matrix(_as_dict(X))).reindex(
        ...
        if isinstance(self._model, LogisticRegression):
            ...
            imp = getattr(self._model, "feature_importances_", np.ones(len(self._columns)))
    ...
    def reason_codes(self, X, top_k: int = 3) -> list[list[ReasonCode]]:
        ...
        for contrib in self.layer_contributions(X):
            ...
            out.append([ReasonCode(source="shap", feature=k, contribution=float(v)) for k, v in ranked if v != 0])
        return out


def _as_dict(X) -> dict[str, Sequence[float]]:
    if isinstance(X, dict):
        return X
    arr = np.asarray(X, dtype=float)
    if arr.ndim == 1:
        arr = arr.reshape(-1, 1)
    return {LAYER_COLUMNS[i]: arr[:, i] for i in range(min(arr.shape[1], len(LAYER_COLUMNS)))}
