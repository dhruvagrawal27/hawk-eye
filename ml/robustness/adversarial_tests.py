...
def default_diverse_ensemble(
    ...
    def unsupervised_member(
        X: pd.DataFrame,
    ) -> pd.Series:  # amount outlier (density signal)
        a = pd.to_numeric(
            X.get("amount_max", pd.Series(0.0, index=X.index)), errors="coerce"
        ).fillna(0.0)
        hi = float(a.quantile(0.95)) or 1.0
        return (a / hi).clip(0, 1)

    def supervised_member(
        X: pd.DataFrame,
    ) -> pd.Series:  # learned proxy: new-bene + privilege
        bene = pd.to_numeric(
            X.get("n_create_beneficiary", pd.Series(0.0, index=X.index)),
            errors="coerce",
        ).fillna(0.0)
    ...
    def graph_member(X: pd.DataFrame) -> pd.Series:  # structural: export/exfil fan-out
        exp = pd.to_numeric(
            X.get("n_export", pd.Series(0.0, index=X.index)), errors="coerce"
        ).fillna(0.0)
    ...
    return DiverseEnsembleDefense(
        members,
        member_thresholds={k: 0.6 for k in members},
        peer_groups=peer_groups,
        secret="battery-fixed-secret",
    )
...
def _check_evasion() -> BatteryResult:
    ...
    evader = "EMP-019"
    X.loc[evader, ["offhours_rate", "amount_max", "n_create_beneficiary"]] = [
        0.05,
        2e4,
        0.0,
    ]
...
def _check_poisoning() -> BatteryResult:
    ...
    peers = {
        f"PEER-{j}": np.full(n, 0.1) + np.random.default_rng(j).normal(0, 0.01, n)
        for j in range(6)
    }
...
def _check_inversion() -> BatteryResult:
    ...
    mon_api = InternalInferenceAPI(
        lambda f: 0.5,
        authorized_tokens={"t": "external"},
        max_calls=1000,
        extraction_monitor=ExtractionMonitor(
            max_queries=4, window_seconds=60.0, dup_ratio_thresh=0.5
        ),
    )
    for t in range(12):
