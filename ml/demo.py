...
def l1_rule_flags(events: pd.DataFrame) -> pd.Series:
    ...
    offh = (
        offh.astype(bool)
        if hasattr(offh, "astype")
        else pd.Series(False, index=df.index)
    )
    verb = df.get("action.verb", "").astype(str)
    hit = ((amount >= 1_000_000) & offh) | (
        (verb == "approve_payment") & (amount >= 1_000_000)
    )
...
def run(employees: int = 150, days: int = 10) -> dict:
    ...
    print("=" * 78)
    print(
        "HAWK-EYE ML — end-to-end walking skeleton (L0 -> L2/L3/L5 -> L6 -> narrative)"
    )
    print("=" * 78)
    ...
    print(
        f"L0: {len(events)} events, {ef.shape[0]} entities, {int(el.sum())} fraud actors "
    ...
    print(
        f"L2 unsupervised: AUPRC {E.average_precision(el.to_numpy(), s_l2.to_numpy()):.3f} "
    ...
    print(
        f"L3 GBDT: AUPRC {E.average_precision(el.to_numpy(), s_l3.to_numpy()):.3f} "
    ...
    print(
        f"L5 XGB-Graph: AUPRC {E.average_precision(el.to_numpy(), s_l5.to_numpy()):.3f} "
    ...
    print(
        f"L6 fusion: AUPRC {E.average_precision(el.to_numpy(), fused.to_numpy()):.3f} "
    ...
    rcs: dict[str, Sequence[ReasonCode]] = {
        "L1": (
            [
                ReasonCode(
                    "rule",
                    code="OFFHOURS_HIGHVALUE_APPROVAL",
                    detail="off-hours approval >= INR 1000000",
                )
            ]
            if rule.loc[top]
            else []
        ...
        "L3": l3_codes,
        "L5": [
            ReasonCode("graph", detail="elevated k-hop neighbourhood risk (XGB-Graph)")
        ],
    }
    alert = fusion.fuse_one(
        ...
        reason_codes_by_layer=rcs,
        exposure_inr=4_800_000,
    )
    ad = alert.to_dict()
    print("\n" + "-" * 78)
    print(
        f"TOP ALERT  entity={ad['entity_id']}  risk={ad['risk_score']}/100  "
    ...
    for rc in ad["reason_codes"]:
        print(
            f"  reason[{rc['source']}]: {rc.get('detail') or rc.get('code') or rc.get('feature')}"
    ...
    print("=" * 78)
    print(
        "END-TO-END OK: L0 -> L2/L3/L5 -> L6 calibrated alert -> grounded narrative -> contestable + reproducible"
    )
