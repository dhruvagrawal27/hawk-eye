...
### 2026-07-01 — [FRONTEND/UI-INTEGRATION] — Merge Daylight-Forensics uplift (A+B+C) onto latest main
Integration branch `hawk-eye/ui-integration` off `origin/main` (297f663). Strategy per owner: **keep main's screens (never remove main's functionality), re-skin via the theme, combine both.** Merged **C** (`ui-integrity`, clean — frontend CI gate + 8 durable safety-invariant guards), **A** (`ui-foundation`, `-X ours` — main wins the 4 conflicting screens; A's light **Daylight Forensics** token theme + `src/ui/**` signature kit + Fraunces/Motion/AutoAnimate deps land and re-skin the whole app via CSS variables), and **B** (`ui-features`, `-X ours` — main keeps its 13 reworked screens; B's non-conflicting uplift folds in: TriageQueue, ReplayStudio, AlertHeader, etc.). The 12 conflicting screens (ExplanationPanel/FusionSankey, GraphExplorer heatmap, ComplianceView UX, AlertDetail gauge, ReportingView, Dashboard, …) were reset to `origin/main` so main's newer functionality is intact and inherits the light theme through tokens. Added the shared **/settings** page (all roles) + re-added **/ui-gallery**. **[FE-proposed]** allowlist extended (contract-routes guard) with main's newer routes not yet in `openapi.json`: `/alerts/stats`, `/entities/{}/risk-index`, `/entities/{}/layer-scores`, `/analytics/typologies`, `/activity/sub-threshold`, `/action-gate/holds`, `/action-gate/holds/{}/decision`, `/action-gate/policies` (10x + L6.5 interdiction; action-gate is the separate :8096 service). Gates green: tsc · **vitest 230** · build. No API shapes changed. Not yet merged to `main` — staged for review.

### 2026-07-01 — [SURFACING] — PHASE 6: Governance & trust — per-layer model lineage on the alert; overhaul COMPLETE
Sixth/final phase of the "show the hidden 95%" overhaul. Gates green (backend **183 pytest** · FE **173 vitest** · tsc · eslint 0 · vite build). Surfaces the reproducibility trail: which model version produced each contributing layer's score, and its governance posture.
- **[BACKEND, additive]** `schemas/explanations.py` adds `ModelLineageEntry` + `Explanation.model_lineage`. `routes/explanation_routes._model_lineage(alert)` builds it from the model registry (`serving/registry.REGISTRY.production_for`) — per layer: `model_id, version` (exact version from `alert.model_versions` when present, else Production), `stage, risk_tier` (MRMF), `signed` (`signed_valid`), `approving_reviewer`, `metrics`; L1 is the deterministic rules engine (four-eyes). **@FRONTEND: `GET /explanations/{id}` now returns `model_lineage[]`.** Test `test_model_lineage.py` (1).
- **[FRONTEND]** new "Model lineage & provenance" section in `ExplanationPanel` (per-layer model_id/version/stage/tier + signature check + N/N-signed trust summary). `types.ts` `ModelLineageEntry` + `ExplanationResponse.model_lineage`. MSW `buildLineage` mirrors the registry into the explanation. Contract assertion added.
- **OVERHAUL COMPLETE (6/6 phases).** The hidden 95% now reaches the screen at all three boundaries: real 5-layer fusion decomposition (P1), variable×temporal attention + SHAP percentile (P2), Entity-360 off-hours heatmap (P3), sub-threshold funnel + watchlist (P4), fraud-typology analytics (P5), per-layer model lineage (P6). Full tracker: `docs/IMPLEMENTATION_PROGRESS.md`. Every phase: schema→service→route→backend test→FE type→apiClient→component→MSW mock→FE test→gates. All invariants preserved (alert-only, RBAC, PII-tokenized, ≥1 reason code); contract strictly additive.

### 2026-07-01 — [SURFACING] — PHASE 5: Management analytics — fraud-typology prevalence + confirmed-rate
Fifth phase. Gates green (backend **182 pytest** · FE **172 vitest** · tsc · eslint 0 · vite build). Portfolio oversight view: *which* insider typologies actually fire, how often they're human-confirmed vs cleared, and the exposure behind each.
- **[BACKEND, new]** `store/analytics_store.py` (`ANALYTICS`, 12-typology synthetic portfolio + detection layers per detection-coverage-map, `seed_analytics()` from `seed_demo`), `schemas/analytics.py`, **new route `GET /api/v1/analytics/typologies`** (VIEW_ALERTS, audited) → `{typologies[typology,label,layers,alerts,confirmed,false_positive,open,confirmed_rate,exposure_inr], totals}`, ranked by prevalence. Registered. Test `test_typology_analytics.py` (2). **@FRONTEND: new endpoint live.**
- **[FRONTEND]** `TypologyAnalytics.tsx` (prevalence bars + colour-coded "% real" confirmed-rate pills + layer chips + compact exposure + totals) mounted on **ReportingView**. `apiClient.getTypologyAnalytics` + `queryKeys.typologyAnalytics` + `types.ts` `TypologyAnalyticsResponse`/`TypologyStat`/`TypologyTotals`. MSW `TYPOLOGY_ANALYTICS` fixture + handler. Tests: `TypologyAnalytics.test.tsx` + contract assertion.

### 2026-07-01 — [SURFACING] — PHASE 4: Sub-threshold visibility — the 'hidden 95%' (detection funnel + near-miss watchlist)
Fourth phase. Gates green (backend **171 pytest** · FE **170 vitest** · tsc · eslint 0 · vite build). Makes the scored-but-not-alerted majority visible: every event runs the full stack but only fused ≥ `EMIT_THRESHOLD` (70) surfaces — everything below was recorded-then-dropped and never shown.
- **[BACKEND, new]** `store/subthreshold_store.py` (`SUBTHRESHOLD` ring + band tallies + funnel counters; `seed_subthreshold()` reseeded from `seed_demo`), `schemas/activity.py` (`SubThresholdResponse`), **new route `GET /api/v1/activity/sub-threshold`** (VIEW_ALERTS, audited) → funnel `{total_scored, alerted, sub_threshold, bands[watch 55–69 / elevated 40–54 / low], watchlist[]}`. `pipeline/online.py` now records **every** fully-scored event (emitted or not) into the store (best-effort, never blocks scoring). Registered in `routes/__init__.py`. Test `test_subthreshold.py` (2). **@FRONTEND: new endpoint live.**
- **[FRONTEND]** `SubThresholdPanel.tsx` (detection-funnel bars + "only N% clears the 70 bar" callout + band breakdown + near-miss watchlist) mounted on the **Dashboard**. `apiClient.getSubThreshold` + `queryKeys.subThreshold` + `types.ts` `SubThresholdResponse`/`WatchlistItem`/`SubThresholdBand`. MSW `SUB_THRESHOLD` fixture + handler. Tests: `SubThresholdPanel.test.tsx` + contract assertion.
- **[flake, not mine]** `tests/unit/test_kill_switch.py::test_disable_unknown_model_404` fails intermittently under full-suite ordering (its model kill-switch store isn't reset in `conftest`); passes in isolation and on re-run. Pre-existing test-isolation gap in the model-registry workstream — flagging for that owner.

### 2026-07-01 — [SURFACING] — PHASE 3: Entity-360 deep dive — 7×24 off-hours activity heatmap
Third phase of the "show the hidden 95%" overhaul. Gates green (backend **160 pytest** · FE **166 vitest** · tsc · eslint 0 · vite build). Surfaces *when* an actor is active and the off-hours share — a first-order insider signal that was buried in the raw timeline.
- **[BACKEND, additive]** `schemas/entities.py` `TimelineEvent` gains `is_off_hours: bool`; `routes/entity_routes.get_timeline` derives it authoritatively from IST bank-hours (Mon–Fri 08:00–20:00) via `_is_off_hours_ist` when the stored row doesn't already carry it. **@FRONTEND: `GET /entities/{id}/timeline` rows now include `is_off_hours`.** Test `test_timeline_offhours.py` (2).
- **[FRONTEND]** new `ActivityHeatmap.tsx` (7-day × 24-hour IST grid; warm = off-hours vs cool = bank-hours, intensity ∝ volume; header stat = off-hours share) mounted in `Entity360Timeline` above the unified timeline, fed from the entity's own events (`context.is_off_hours` when present, else IST bank-hours). Test `ActivityHeatmap.test.tsx` (4).
- **Deferred (logged in tracker):** per-layer score timeline from `hawkeye.scores` (bigger backend/ClickHouse surface) — folded into a later polish pass; the activity heatmap is the Phase-3 flagship.

### 2026-07-01 — [SURFACING] — PHASE 2: Rich explainability — L4 attention heatmap (variable×temporal) + SHAP peer percentile
Second phase of the "show the hidden 95%" overhaul (tracker: `docs/IMPLEMENTATION_PROGRESS.md`). All gates green (backend **158 pytest** · FE **162 vitest** · tsc · eslint 0 · vite build). The L4 attention that was collapsed to a single temporal bar is now the full LAXCAT **variable×temporal matrix**, and SHAP features carry a **peer percentile** ("vs baseline" depth).
- **[BACKEND, additive]** `schemas/explanations.py`: `AttentionStep` gains `variables: [{name,weight}]` (the variable axis); `ShapFeature` gains `percentile`. `routes/explanation_routes.py` synthesizes both (`_attention_variables` per step over `verb/log_amount/off_hours/velocity_1h`; percentile from the signed SHAP contribution) — honest heuristics, `# STUB: ML` marked, ready to swap for the real LAXCAT variable-attention + peer stats. **@FRONTEND: `GET /explanations/{id}` now returns `sequence_attention[].variables[]` and `shap[].percentile`.**
- **[FRONTEND]** new `AttentionHeatmap.tsx` (rows=variables, cols=session steps, cell=variable-attention×temporal, header bar=temporal attention) — `AttentionView` auto-promotes to it when per-variable data is present, else keeps the temporal bars. `ShapChart` tooltip shows value + `pNN (vs comparable peers)`. `types.ts` `AttentionStep.variables`/`AttentionVariable` + `ShapContribution.percentile` (additive). MSW `handlers.ts` enriches fixture+fallback (`stepVariables`/`enrichShap`). Tests: `AttentionHeatmap.test.tsx` (4) + attention/percentile assertions in `payloads.contract.test.ts`; backend `test_explanation_richness.py` (2).
- **Note:** structured L5 graph evidence already renders as node/edge chips in `GraphEvidenceSummary` (not raw text); a full inline mini-graph SVG is deferred as a later polish item (logged in the tracker), not blocking.

### 2026-07-01 — [PLATFORM] — FIX: Grafana ops dashboard "No data" (Prometheus not scraping + embed time-range trap)
**Symptom:** the embedded Operations dashboard (`/d/hawk-eye-ops`) showed **No data** on every panel. **Root cause (two):** (1) the *running* brew Prometheus had a **stale config with zero scrape targets** — it was started before `deploy/local-observability/up.sh` copied `prometheus.yml` and never restarted, so it never scraped `localhost:8000/metrics` (backend WAS up and emitting `hawkeye_http_requests_total{status="2xx"}` correctly). (2) The embed loaded `/d/hawk-eye-ops?kiosk` with **no time range**, so Grafana could fall back to a persisted "Previous year" window (the screenshot) → no samples. **Fix:** reinstalled the scrape config to `$(brew --prefix)/etc/prometheus.yml` + `brew services restart prometheus` — verified `hawkeye-backend` target now **up** and 5 series returned. Code: `GrafanaEmbed` default path now pins `&from=now-30m&to=now&refresh=10s`. Canonical setup remains `bash deploy/local-observability/up.sh`. @PLATFORM: dashboards query `hawkeye_http_requests_total`/`hawkeye_http_request_duration_seconds_bucket` with `status` bucketed `2xx/4xx/5xx`.

### 2026-07-01 — [10×] — M2.1 continuous per-user insider-risk index (pipeline + endpoint)
- **ML batch (`ml/pipelines/insider_risk_index.py`).** `compute_insider_risk_index(events, alerts_30d)` → per-employee `InsiderRiskIndex` (0–100 composite from HR 30% / access 35% / anomaly 35% sub-scores, with per-component values, details, and top-3 drivers). Composes EXISTING features (`no_leave_taken_streak`, `grievance_recency` [M1.5], `role_change_recency`, leaver/notice, `entitlement_change_velocity`, `standing_privilege` [M2.2], `dormant_reactivation`, `offhours_score`, recent-alerts-30d). Saturating normalization (peer-relative z is the documented calibration follow-on). **Weights are STUBS** → `calibrated=false`; alert-only, human-review-only, never auto-actioned.
- **Backend read side (no ML dep).** `schemas/risk_index.py` (`RiskIndexResponse`), `EntityStore.put/get_risk_index`, `GET /entities/{id}/risk-index` (VIEW_ALERTS + `entity.risk-index` audit, 404 when absent), demo index seeded for EMP-7f3a. The ML batch WRITES, backend READS (decoupled). Tests: `tests/ml/test_insider_risk_index.py` (5), `backend/tests/contract/test_risk_index.py` (3). **Frontend gauge = pending.**
- **[FIX]** latent bug in `change_hr.role_change_recency` — `grp.get(TENURE)` returned a scalar (crash) when `actor.tenure_days` absent; now column-guarded.

### 2026-07-01 — [SURFACING] — "Show the hidden 95%" overhaul — PHASE 1: Detection Transparency (real 5-layer fusion decomposition)
Kick-off of the phased UI/UX + data-surfacing overhaul (tracker: `docs/IMPLEMENTATION_PROGRESS.md`). Diagnosis: the engine's richness leaks at 3 boundaries — ML→alert truncation, storage→API (EMIT_THRESHOLD=70 drops sub-70 to ClickHouse unseen; `hawkeye.scores`/typology/disposition analytics never aggregated), API→UI under-visualization. Mandate: broad overhaul, all personas, balanced wow+utility, backend free to add/enrich, **phased with gates**, on the existing apiClient + MSW-mock/live-toggle seam. Invariants preserved (alert-only, RBAC, PII-tokenized, ≥1 reason code).
- **Phase 1 shipped (all gates green: backend 153 pytest · FE 157 vitest · tsc · eslint 0 · vite build):** the L6 fusion now emits a **transparent per-layer breakdown** and it flows to L7.
  - **[BACKEND]** `fusion/service.py`: `FusionOutput` gains `layer_scores`/`sod`/`agreement`/`breakdown`; new `build_breakdown()` produces the canonical 5-layer decomposition — each layer's raw 0–1 score, meta coefficient (`LAYER_META_WEIGHTS`: L1 1.7 · L2 1.2 · L3 2.4 · L4 1.0 · L5 1.2), weighted contribution, cross-layer **agreement** (factored out to `calibration.agreement_from`), fused prob vs **decision threshold** (=EMIT/100=0.70), and an honest **decisive-layer / rescued-by** counterfactual. Attached to the alert (`Alert.fusion_breakdown`, internal/excluded) across **all three emit paths** (fused · L1 short-circuit · degraded) in `pipeline/online.py`.
  - **[BACKEND contract — additive]** `GET /explanations/{id}` now returns a **`fusion`** object (`schemas/explanations.py` `FusionBreakdown`/`FusionComponent`); real alerts carry the stored breakdown, legacy/seeded alerts get an honest fallback synthesis. No existing field removed. **@FRONTEND: `ExplanationResponse.fusion` is now real (5 layers, agreement, rescued, decisive_layer) — not FE-synthesized.**
  - **[FRONTEND]** `types.ts` `FusionBreakdown`/`FusionComponent`/`FusionLayer` extended (additive: +L1_rule/L4_sequence, +contribution/agreement/rescued/decisive_layer/…). `ScoreComposition` now renders all 5 layers + a **cross-layer-agreement** bar + a decisive-layer-aware rescue callout, sourced from the backend. `layerFusion.deriveLayerBreakdown` normalizes the `L1_rule`↔`L1_rules` key split and adopts the backend threshold/rescued/decisive/agreement. MSW `buildFusion` mirrors the backend 5-layer shape (worked burst `alr_3d7e22` still tells the canonical "rescued by graph fusion" story). Tests: `ScoreComposition.test.tsx` + fusion assertion in `payloads.contract.test.ts`.
