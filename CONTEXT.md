...
### 2026-07-01 — [FRONTEND/UI-B] — Feature-screen uplift to "Daylight Forensics" (branch hawk-eye/ui-features)

Re-skinning all 12 investigator screens to A's Daylight Forensics kit — **consumes `@/ui` only; no theme/tokens/shell/harness edits** (A + C lanes). Per-screen recipe: wrap each view in `RouteTransition`, editorial `font-display` headings + `font-mono` for all evidence (IDs/hashes/₹/ts), compose signature primitives (`RiskGauge`, `AmountFlip`, `SlaRing`, `PeerStrip`, `HashChainBlock`, `EventTicker`, `Sparkline`), `useAutoAnimateList` for live lists, staggered reveal on load, and first-class loading/empty/error. **Invariants kept literal & more visible:** reason codes always rendered (explicit _"no reasons — invalid"_ state, never blank), alert-only (human `block-request` → lead `approve`; nothing auto-blocks/auto-classifies), audited unmask via `MaskedPII` (fires audit + "logged" toast — **left to A** per UI_UPLIFT §4, composed unchanged here), RBAC/SoD gating unchanged, no invented API fields (bind to `@/lib/types`). Adding e2e for reason-codes-present / unmask-logged / no-auto-block. Gates target: vitest ≥149, e2e ≥3, tsc/eslint/prettier/build green. Log + crib: `docs/laptops/ui-features.md`. `[A]` requests (if any) go in `UI_UPLIFT.md`.

### 2026-07-01 — [FRONTEND/UI-A] — "Daylight Forensics" UI uplift kicked off (branch hawk-eye/ui-foundation)

Design-system foundation for the light‑theme UI uplift. Contract lives in **`docs/ui/UI_UPLIFT.md`** (tokens + motion + primitive API; A owns, B/C read) and ownership in `docs/ui/OWNERSHIP.md`. Shipping: the light **Daylight Forensics** token set (warm porcelain/bone/paper canvas, warm ink, **deep ink‑teal** accent — no SaaS purple, sacred colorblind‑safe risk ramp), a Motion‑v12 + AutoAnimate **reduced‑motion‑aware** motion foundation, signature primitives (`RiskGauge`, `AmountFlip`, `CountUp`, `SlaRing`, `Sparkline`, `PeerStrip`, `HashChainBlock`, `EventTicker`) under **`frontend/src/ui/`**, and a mock‑only **`/ui-gallery`**. Light is now the default (`.dark` retired). New deps: `motion`, `@formkit/auto-animate`, `@fontsource-variable/fraunces`. **Invariants preserved** (alert‑only, contestability, audited unmask, RBAC/SoD, MSW mocks, no invented API fields). Gates held: vitest ≥149, e2e ≥3. Feature screens (B) + verification harness (C) are separate branches; nothing here changes API shapes (`backend/openapi.json`).
