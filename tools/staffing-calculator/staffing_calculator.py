...
def service_level(
    offered_load: float, agents: int, target_wait_h: float, aht_h: float
) -> float:
...
def compute(
    ...
    while (
        service_level(offered_load, agents, sla_hours, aht_h) < target_sl
        and agents < 1000
    ):
        agents += 1
    ...
    return Staffing(
        ...
        concurrent_agents_required=agents,
        achieved_service_level=round(achieved, 4),
        occupancy=round(occupancy, 3),
        fte_with_shrinkage=fte,
        roster=roster,
    )
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Investigator staffing calculator (Part 33.3)"
    )
    ...
    a = ap.parse_args()
    s = compute(
        a.alerts_per_day, a.handling_min, a.sla_hours, a.coverage_hours, a.target_sl
    )
    if a.json:
        print(json.dumps(asdict(s), indent=2))
        return 0
    print(
        f"Staffing — {a.alerts_per_day} alerts/day, {a.handling_min} min handling, "
        f"SLA {a.sla_hours}h, target SL {int(a.target_sl*100)}%"
    )
    print(f"  offered load        : {s.offered_load_erlangs} Erlangs")
    print(
        f"  concurrent agents   : {s.concurrent_agents_required} (SL {int(s.achieved_service_level*100)}%, "
        f"occupancy {int(s.occupancy*100)}%)"
    )
    print(
        f"  FTE (w/ shrinkage)  : {s.fte_with_shrinkage} across {s.roster['shifts']} shifts "
