...
def score_event(
    ...
    if rule_hits:
        ...
        reason_codes.extend({k: v for k, v in rc.items() if k != "severity"} for rc in rule_hits)
    ...
    confidence = 0.6 if mode == "rules_only" else round(min(0.95, 0.6 + 0.1 * len(contributing)), 2)
    return {
