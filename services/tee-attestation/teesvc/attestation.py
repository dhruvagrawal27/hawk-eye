...
def _canonical(report: dict) -> bytes:
    ...
    signed = {k: report[k] for k in
              ("provider", "model", "prompt_hash", "ts", "enclave_mode", "cpu_quote", "gpu_quote")}
    return json.dumps(signed, sort_keys=True, separators=(",", ":")).encode()


def issue_quote(prompt: str, model: str, provider: str, enclave_mode: bool,
    ...
    if leak:
        raise ValueError(f"PII not tokenized before egress (matched /{leak}/); refusing attestation")

    nonce = nonce or hashlib.sha256(f"{prompt}{time.time_ns()}".encode()).hexdigest()[:16]
    prompt_hash = hashlib.sha256((prompt or "").encode()).hexdigest()[:16]
    report_data = hashlib.sha256(f"{prompt_hash}|{nonce}".encode()).hexdigest()

    cpu_quote = {
        "tee": "intel-tdx", "type": "TDX_QUOTE", "fmspc": "00606A000000",
        "mrtd": _measurement("intel-tdx", nonce, model),
        "report_data": report_data, "nonce": nonce, "tcb_status": "UpToDate",
    }
    gpu_quote = {
        "tee": "nvidia-h200", "type": "NV_GPU_ATTESTATION", "arch": "Hopper",
        "measurement": _measurement("nvidia-h200", nonce, model),
        "report_data": report_data, "nonce": nonce, "driver_attested": True,
    }
    report = {
        "provider": provider,
        "model": model,
...
def verify_quote(report: dict) -> dict:
    ...
    if not sig:
        return {"valid": False, "reasons": ["missing signature"], "attestation_id": report.get("attestation_id")}
    try:
    ...
    return {"valid": not reasons, "reasons": reasons or ["ok"],
