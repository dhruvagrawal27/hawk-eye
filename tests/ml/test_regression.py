...
from ml.eval import average_precision, is_temporal_split, precision_at_k, recall_at_k
...
PRECISION_AT_K_FLOOR = 0.10  # observed ~0.14 at k=2x positives; floor >> 0.011 prevalence
...
def test_auprc_floor_holds(scored):
    ...
    assert ap > prevalence, f"AUPRC {ap:.4f} must beat random prevalence {prevalence:.4f}"
    assert ap >= AUPRC_FLOOR, (
...
def test_precision_at_k_floor_holds(scored):
    ...
    assert pk >= PRECISION_AT_K_FLOOR, (
...
def test_previously_caught_cases_stay_caught(scored):
    ...
    assert known.size > 0
    thr = float(np.quantile(p, 1.0 - TEST_FRAC))  # operating point near the alert budget
...
def test_eval_is_deterministic_under_fixed_seed(split):
    ...
    assert aps[0] == pytest.approx(aps[1], abs=1e-9), "fixed-seed eval must be reproducible"
