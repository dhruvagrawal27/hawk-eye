...
def test_peer_relative_scoring_normalises_within_group():
    ...
    scores = pd.Series([10, 11, 12, 50, 100, 101, 102, 140],
                       index=[f"E{i}" for i in range(8)], dtype=float)
...
def test_peer_group_from_features_uses_data_peer_group():
    ...
    ev = pd.DataFrame({"actor.employee_id": ["EMP-1", "EMP-2"],
