...
from ml.layers.l6 import L6Fusion, StackedMetaLearner, assemble_layer_matrix, build_alert
...
def _layer_scores(n=1200, pos=80, seed=0):
    ...
    df = pd.DataFrame({
        "L1_rule": (rng.random(n) < (0.6 * y + 0.02)).astype(float),  # fires on most frauds + few FPs
...
def test_meta_learner_is_transparent_and_consumes_all_layers():
    ...
    assert set(contribs[0].keys()) == set(LAYER_COLUMNS)  # every per-layer score + rule consumed
...
def test_fused_score_beats_best_single_layer():
    ...
    ap_single = max(E.average_precision(y.iloc[te].to_numpy(), X.iloc[te][c].to_numpy()) for c in LAYER_COLUMNS)
    ap_rand = E.average_precision(y.iloc[te].to_numpy(), np.random.default_rng(9).random((te.stop or len(y)) - te.start))
    assert ap_fused > ap_rand
    assert ap_fused >= ap_single - 0.05  # stacking is at least competitive with the best single layer
...
def test_fuse_one_matches_backend_alert_shape():
    ...
    alert = fusion.fuse_one(
        entity_id="EMP-7f3a", alert_id="alr_3d7e22",
        layer_scores={"L1_rule": 1.0, "L2_unsupervised": 0.8, "L3_gbdt": 0.9, "L4_sequence": 0.3, "L5_graph": 0.7},
        reason_codes_by_layer={
            "L1": [ReasonCode("rule", code="NEW_BENEFICIARY_THEN_HIGHVALUE", detail="new payee paid within 27 min")],
            "L3": [ReasonCode("shap", feature="new_bene_latency_min", contribution=0.31)],
            "L5": [ReasonCode("graph", detail="maker+checker recur as isolated pair (ring RNG-12)")],
        },
    ...
    assert all(rc["source"] in ("rule", "shap", "graph", "attention") for rc in d["reason_codes"])
...
def test_severity_banding_in_alert():
    a = build_alert(alert_id="a", entity_id="e", calibrated_prob=0.95, confidence=0.9,
                    contributing_layers=["L3_gbdt"], reason_codes=[])
