...
BENIGN = {
    ...
    "object": {"amount": 0},
    "context": {"is_off_hours": False, "layer": "application"},
}
...
def test_swift_cbs_mismatch_rule():
    ev = {
        "action": {"channel": "swift"},
        "linkage": {"cbs_entry_present": False},
        "object": {},
    }
...
def test_full_mode_fuses_layers():
    a = scoring.score_event(
        FRAUD, "full", {"L2_unsupervised": 0.82, "L3_gbdt": 0.78, "L5_graph": 0.7}
    )
...
def test_benign_suppressed_both_modes():
    ...
    assert (
        scoring.score_event(BENIGN, "full", {"L2_unsupervised": 0.05, "L3_gbdt": 0.05})
        is None
    )
