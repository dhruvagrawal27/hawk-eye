...
def test_attest_then_verify_roundtrip():
    rep = attestation.issue_quote(
        "Alert EMP-7f3a paid BEN-9b1c off-hours",
...
def test_tampered_quote_fails_verify():
    rep = attestation.issue_quote(
        "tokenized prompt", model="m", provider="near_ai", enclave_mode=True
    )
...
def test_enclave_mode_false_fails_verify():
    rep = attestation.issue_quote(
        "p", model="m", provider="near_ai", enclave_mode=False
    )
    v = attestation.verify_quote(rep)
    assert v["valid"] is False
